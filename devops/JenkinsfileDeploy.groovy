            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "demo-533 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "demo-533 failed to authenticate properly"
                throw err
            }
        }
    }
        ...
        try {
            dir(projectDirectory) {
                ...
                slackSend color: "good",
                        message: "demo-533 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "demo-533 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "demo-533 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "demo-533 failed to shut down properly"
            throw err
        }
    }
}
