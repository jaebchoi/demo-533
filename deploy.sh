...
startup() {
  ...
  helm upgrade --install $INFRA_NAME demo-533-infrastructure \
     --values demo-533-infrastructure/values.yaml \
     --values demo-533-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/demo-533 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "demo-533 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying demo-533 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace demo-533 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/demo-533 \
      --path demo-533-deploy/src/main/resources \
      --revision $branch \
